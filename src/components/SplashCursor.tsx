...
export const SplashCursor: React.FC<SplashCursorProps> = ({
  ...
  useEffect(() => {
    ...
    function getWebGLContext(canvasEl: HTMLCanvasElement) {
      ...
      let glCtx: WebGLRenderingContext | WebGL2RenderingContext | null = canvasEl.getContext("webgl2", params) as WebGL2RenderingContext | null;
      const isWebGL2 = !!glCtx;
      if (!isWebGL2) {
        glCtx = (canvasEl.getContext("webgl", params) || canvasEl.getContext("experimental-webgl", params)) as WebGLRenderingContext | null;
      }
      ...
      const halfFloatTexType = isWebGL2 ? (glCtx as any).HALF_FLOAT : halfFloat && halfFloat.HALF_FLOAT_OES;
      let formatRGBA: any;
      let formatRG: any;
      let formatR: any;
      ...
      if (isWebGL2) {
        ...
        formatRGBA = getSupportedFormat(glCtx, gl2.RGBA16F, gl2.RGBA, halfFloatTexType);
        ...
        const gl1 = glCtx as any;
        formatRGBA = getSupportedFormat(glCtx, gl1.RGBA, gl1.RGBA, halfFloatTexType);
        formatRG = getSupportedFormat(glCtx, gl1.RGBA, gl1.RGBA, halfFloatTexType);
        formatR = getSupportedFormat(glCtx, gl1.RGBA, gl1.RGBA, halfFloatTexType);
      }
    ...
    function generateColor() {
      if (config.COLORS && config.COLORS.length > 0) {
        if (config.COLORS.length === 1) {
          return hexToRGB(config.COLORS[0]);
        }
        // Smooth sine-wave blending between brand colors (e.g., Cyan and Purple)
        const palette = config.COLORS.map(hexToRGB);
        const t = (Math.sin(colorPhase) + 1) / 2;
        colorPhase += 0.35;
        const c1 = palette[0];
        const c2 = palette[1];
        return {
